...
Move a [base](/reference/components/base/) component to a destination [pose](/operate/mobility/orientation-vector/) on a {{< glossary_tooltip term_id="slam" text="SLAM" >}} map.
...
To use `MoveOnMap()`, your [SLAM service](/reference/services/slam/) must implement `GetPointCloudMap()` and `GetPosition()`

Make sure the [SLAM service](/reference/services/slam/) you use alongside this motion service supports the following methods in its {{< glossary_tooltip term_id="model" text="model's" >}} implementation of the [SLAM service API](/reference/apis/services/slam/):
...
- `slam_service_name` ([str](https://docs.python.org/3/library/stdtypes.html#text-sequence-type-str)) (required): The `ResourceName` of the [SLAM service](/reference/services/slam/) from which the SLAM map is requested.
...
- `SlamName` [(resource.Name)](https://pkg.go.dev/go.viam.com/rdk/resource#Name): The `resource.Name` of the [SLAM service](/reference/services/slam/) from which the SLAM map is requested.
