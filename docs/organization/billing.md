...
| ---- | ---- | --------- | ----------- |
| `cost_per_month` | object | Optional | See [cost per month attributes](/organization/billing/#click-to-view-cost-per-month-attributes). If specified, you cannot also specify `cost_per_year`. Default: `{}` (all machines cost `0`). |
| `cost_per_year` | object | Optional | See [cost per year attributes](/organization/billing/#click-to-view-cost-per-year-attributes). If specified, you cannot also specify `cost_per_month`. Default: `{}` (all machines cost `0`). |
