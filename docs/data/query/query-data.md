...
Binary data such as images and point clouds is stored separately and is not queryable via SQL/MQL.
...
3. Use the field names from step 2 to write a query that extracts a specific value with dot notation (e.g., `data.readings.temperature`).
...
| `method_name` | String | Capture method (e.g., `Readings`, `EndPosition`) |
...
Use dot notation in queries to reach into nested fields (e.g., `data.readings.temperature`).
