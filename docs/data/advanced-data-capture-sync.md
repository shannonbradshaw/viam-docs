...
| `additional_sync_paths` | string array | Optional | Paths to any other directories on your machine from which you want to sync data to the cloud. Once data is synced from a directory, it is automatically deleted from your machine. We recommend using absolute paths. For relative paths, see [How sync works](/data/how-sync-works/#cant-find-the-directory-data-is-stored-in-click-here). |  |
...
The following example shows the configuration of the part that we will establish as a remote, in this case an [ESP32 board](/reference/components/board/esp32/).
