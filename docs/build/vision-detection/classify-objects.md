...
|-------|------|-------------|
| `class_name` | String | The label assigned by the model (e.g., "cat", "dog", "empty") |
...
func main() {
	...
	classifier, err := vision.FromRobot(robot, "my-classifier")
	if err != nil {
		logger.Fatal(err)
	}
...
classifications = await classifier.get_classifications(image, count=5)
...
classifications, err := classifier.Classifications(ctx, images[0].Image, 5, nil)
if err != nil {
    logger.Fatal(err)
}
...
- If all classes show roughly equal confidence (e.g., five classes each at ~0.2), the model is essentially guessing. The input image likely does not match anything in the training set.
...
- [Detect Objects (2D)](/build/vision-detection/detect-objects-2d/) -- get per-object bounding boxes instead of whole-image labels.
- [Track Objects Across Frames](/build/vision-detection/track-objects-across-frames/) -- maintain object identities across consecutive frames.
- [Trigger on Detection](/build/stationary-vision/trigger-on-detection/) -- use classification or detection results to trigger automated actions.
