...
- The model may need more training data. See [Train a Model](/build/train/train-a-model/) for improving model quality.
...
- [Detect Objects (2D)](/build/vision-detection/detect-objects-2d/) -- use the vision service to get bounding boxes, filter by confidence, and process detection results in code.
- [Classify Objects](/build/vision-detection/classify-objects/) -- use the vision service for whole-image classification instead of per-object detection.
- [Train a Model](/build/train/train-a-model/) -- train a custom model on your own data for better accuracy on your specific use case.
