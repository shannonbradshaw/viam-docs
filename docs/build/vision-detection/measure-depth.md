...
func main() {
	...
	cam, err := camera.FromRobot(robot, "my-depth-camera")
	if err != nil {
		logger.Fatal(err)
	}
...
# Get the depth image
depth_image = await camera.get_image(mime_type="image/vnd.viam.dep")
...
// Get the depth image
img, release, err := cam.Stream(ctx)
if err != nil {
    logger.Fatal(err)
}
defer release()

frame, err := img.Next(ctx)
if err != nil {
    logger.Fatal(err)
}

fmt.Printf("Depth image retrieved\n")
```
...
// The depth image pixel values represent distance in mm
depthImg := images[1].Image // Index 1 is typically the depth image
...
- If using a module (e.g., the RealSense module), verify the module is installed and the camera is configured to output depth data.
...
- [Localize Objects in 3D](/build/vision-detection/localize-objects-in-3d/) -- project 2D detections into 3D space using the depth data from this how-to.
- [Detect Objects (2D)](/build/vision-detection/detect-objects-2d/) -- get 2D detections to combine with depth measurements.
- [Define Your Frame System](/build/work-cell-layout/define-your-frame-system/) -- understand how coordinate frames relate to each other when placing depth data in the context of your full robot.
