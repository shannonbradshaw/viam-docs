...
Open the generated model file (e.g., `src/models/safe-arm.py`) and add imports for the vision service and any resources you will control:
...
3. Enter the path to `run.sh` (e.g., `/home/user/my-vision-module/run.sh`).
...
2. Fill in:
   ...
   - **Type**: the resource type (e.g., `arm`)
   - **Name**: a descriptive name (e.g., `safe-arm-1`)
