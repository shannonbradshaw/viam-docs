...
- **A label** -- the detection or classification class name to look for (e.g., "Person", "NO-Hardhat").
...
6. Set the notification frequency (e.g., maximum one alert per hour).
7. Click **Save**.
...
- Lower the data capture frequency (e.g., from `0.2` to `0.1` images per second).
- Increase the notification frequency limit in the trigger settings (e.g., maximum one per hour).
