...
| `remap_output_names` | object | Optional | Map model output tensor names to the names the vision service expects (e.g., `location`, `category`, `score`). Use when the model's output names differ from the expected names. |
...
| `input_image_mean_value` | array of float | Optional | Per-channel mean values for input image normalization (e.g., `[127.5, 127.5, 127.5]`). Subtracted from each pixel before inference. |
| `input_image_std_dev` | array of float | Optional | Per-channel standard deviation values for input normalization (e.g., `[127.5, 127.5, 127.5]`). Each pixel is divided by this after mean subtraction. |
