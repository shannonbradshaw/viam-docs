...
4. Search for **gamepad**. This is the built-in model for USB game
   controllers.
5. Click **Add component**, name it (e.g., `my-gamepad`), and click
...
4. Click **Add component**, name it (e.g., `web-controller`), and click
   **Add component** again to confirm.
...
3. Search for **gpio input controller**. This model maps GPIO pins to
   controller buttons.
4. Click **Add component**, name it, and click **Add component** again
   to confirm.
...
func main() {
	...
	controller, err := input.FromRobot(robot, "my-gamepad")
	if err != nil {
		logger.Fatal(err)
	}
