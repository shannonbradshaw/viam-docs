...
However, if you are unable to use [modular resources](/build-modules/write-a-driver-module/) because you need to host `viam-server` on a non-Linux system or have an issue with compilation, you can use a [Viam SDK](/reference/sdks/) to code a custom resource implementation, host it on a server, and add it as a [remote part](/reference/architecture/parts/) of your machine.
...
Then you can control it as part of your machine with the same [API methods](/reference/apis/components/arm/#api) available for [arm models built into the RDK](/reference/components/arm/#configuration).

## Instructions

To add a custom resource as a [remote part](/reference/architecture/parts/):
...
3. Add the server as a [remote part](/reference/architecture/parts/) of your machine.
...
1. Add the server as a [remote part](/reference/architecture/parts/) of your machine.
