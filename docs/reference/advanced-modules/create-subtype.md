...
For example, if you have a [sensor](/reference/components/sensor/) and you want to define a `Calibrate` method, you can use `DoCommand`.

If your use case uses only `DoCommand` and no other API methods, you can define a new model of [generic component](/reference/components/generic/) or [generic service](/reference/services/generic/).
...
1. Choose a name for your API.
   ...
   You can figure out the {{< glossary_tooltip term_id="model-namespace-triplet" text="model namespace triplet" >}} later when you [create a model that implements your custom API](/build-modules/write-a-driver-module/).
...
{{< cards >}}
{{% manualcard link="/build-modules/write-a-driver-module/" %}}
