...
[`GetImage`](/reference/apis/components/camera/#getimage) is deprecated.
Use [`GetImages`](/reference/apis/components/camera/#getimages) instead.
...
Instead use [BinaryDataByFilter](/reference/apis/data-client/#binarydatabyfilter), [AddTagsToBinaryDataByIDs](/reference/apis/data-client/#addtagstobinarydatabyids), and [RemoveTagsFromBinaryDataByIDs](/reference/apis/data-client/).
...
You can store and retrieve arbitrary metadata about your organization, location, machine, and machine part with the [Fleet Management API](/reference/apis/fleet/).
...
For updated Go usage information, see [`GetImage`](/reference/apis/components/camera/#getimage).
...
- All users adding [custom APIs](/reference/advanced-modules/create-subtype/)
...
Two new component APIs, the [button API](/reference/apis/components/button/) and [switch API](/reference/apis/components/switch/), have been added.

{{% /changelog %}}

{{% changelog color="added" title="Discovery service API" date="2025-01-27" %}}

The [discovery service API](/reference/apis/services/discovery/) has been added.
...
Instead, use a [discovery service](/reference/services/discovery/) to discover resources on a machine.
...
The [arm interface](/reference/apis/components/arm/) now includes a [MoveThroughJointPositions](https://pkg.go.dev/go.viam.com/rdk/components/arm#Arm) method that moves an arm through an ordered array of joint positions.
{{% /changelog %}}
...
For more information, see [Configure an esp32-camera](/reference/components/camera/esp32-camera/).
...
For more information, see [viam-micro-server](/reference/platform/viam-micro-server/).
...
The `errors_only` parameter has been removed from [`get_robot_part_logs()`](/reference/apis/fleet/#getrobotpartlogs) and replaced with `log_levels`.

{{% /changelog %}}

{{% changelog date="2024-05-28" color="changed" title="Return type of analog Read" %}}

The board analog API [`Read()`](/reference/apis/components/board/#readanalogreader) method now returns an `AnalogValue` struct instead of a single int.
...
The vision service now supports two new methods: [`CaptureAllFromCamera`](/reference/apis/services/vision/#captureallfromcamera) and [`GetProperties`](/reference/apis/services/vision/#getproperties).
...
This affects users of the [`MoveOnGlobe()`](/reference/apis/services/motion/#moveonglobe) method.
...
The camera component's [`GetImage()`](/reference/apis/components/camera/#getimage) method now returns a `ViamImage` type, and the vision service's [`GetDetections()`](/reference/apis/services/vision/#getdetections) and [`GetClassifications()`](/reference/apis/services/vision/#getclassifications) methods take in `ViamImage` as a parameter.
...
Use [`AnalogByName()`](/reference/apis/components/board/#analogbyname) followed by [`Write()`](/reference/apis/components/board/#writeanalog) instead.

{{% /changelog %}}

{{% changelog date="2024-04-30" color="changed" title="Python SDK data retrieval behavior" %}}

[`tabular_data_by_filter()`](/reference/apis/data-client/#tabulardatabyfilter) and [`binary_data_by_filter()`](/reference/apis/data-client/#binarydatabyfilter) now return paginated data.
...
`AnalogReaderByName()` and `AnalogReaderNames()` have become [`AnalogByName()`](/reference/apis/components/board/#analogbyname) and `AnalogNames()` (since deprecated), respectively.
...
In addition to the existing returned metadata, the [`GetCloudMetadata`](/reference/apis/robot/#getcloudmetadata) method now returns `machine_id` and `machine_part_id` as well.
...
You can now use the [ultrasonic sensor component](/reference/components/sensor/ultrasonic-micro-rdk/) with [`viam-micro-server`](/reference/platform/viam-micro-server/) to integrate an [HC-S204](https://www.sparkfun.com/products/15569) ultrasonic distance sensor into a machine running `viam-micro-server`.
...
You can now use the [Viam Agent](/reference/platform/viam-agent/) to provision your machine or fleet of machines during deployment.
...
You can now use the [generic service](/reference/components/generic/) to define new, unique types of services that do not already have an [appropriate API](/reference/apis/#service-apis) defined for them.
...
Viam has added a [sensor-controlled base](/reference/components/base/sensor-controlled/) component model, which supports a robotic base that receives feedback control from a movement sensor.
...
You can now configure [triggers](/data/trigger-on-data/) (previously called webhooks) to execute actions when certain types of data are sent from your machine to the cloud.
...
Check out [this guide](/data/filter-at-the-edge/#use-a-filtered-camera-with-ml) for more information.
...
For more information, see [Test classification models with existing images in the cloud](/reference/services/vision/mlmodel/#existing-images-in-the-cloud).
...
The [Cartographer-module](/reference/services/slam/cartographer/) now runs in Viam's cloud for creating or updating maps.
...
You now have the capability to use a [power sensor component](/reference/components/power-sensor/) to monitor the voltage, current, and power consumption within your machine's system.
...
Arm models now support the [`GetKinematics` method](/reference/apis/components/arm/#getkinematics) in the arm API, allowing you to request and receive kinematic information.
...
[Session management](/reference/apis/sessions/) helps you to ensure safer operation of your machine when dealing with actuating controls.
Sessions are enabled by default, with the option to [disable sessions](/reference/apis/sessions/#disable-default-session-management).
...
This update enables you to implement custom models of an arm component as a modular resource by coding three endpoints of the [Arm API](/reference/apis/components/arm/#api):
...
- Specify speed values when calling the `MovetoPosition` method on [Gantry components](/reference/components/gantry/).
...
- The ability to hide or display your machines' [secrets](/reference/apis/)
...
The [vision service](/reference/services/vision/) became more modular in RDK [v0.2.36](https://github.com/viamrobotics/rdk/releases/tag/v0.2.36), API [v0.1.118](https://github.com/viamrobotics/api/releases/tag/v0.1.118), and Python SDK [v0.2.18](https://github.com/viamrobotics/viam-python-sdk/releases/tag/v0.2.18).
...
You can replace existing color detectors by [configuring new ones in the UI](/reference/services/vision/color_detector/) or you can update the JSON configuration of your machines:
...
You can replace existing TFLite detectors by [configuring new ones in the UI](/reference/services/vision/mlmodel/) or you can update the JSON configuration of your machines:
...
You can replace existing TFLite classifiers by [configuring new ones in the UI](/reference/services/vision/mlmodel/) or you can update the JSON configuration of your machines:
...
You can replace existing Radius Clustering 3D segmenters by [configuring new ones in the UI](/reference/services/vision/detector_3d_segmenter/) or you can update the JSON configuration of your machines:
...
Find more information in the [`viam-micro-server` docs](/reference/platform/viam-micro-server/).

{{% /changelog %}}

{{% changelog date="2023-01-31" color="added" title="Remote control power input" %}}

On your machine's **CONTROL** tab, you can now set the power of a [base](/reference/components/base/).
...
The movement sensor API now includes a [GetLinearAcceleration](/reference/apis/components/movement-sensor/#getlinearacceleration) method.
...
For information on configuring any camera model, see [Camera Component](/reference/components/camera/).

{{% /changelog %}}

{{% changelog date="2022-11-15" color="added" title="New servo model" %}}

A new [servo model called `gpio`](/reference/components/servo/gpio/) supports servos connected to non-Raspberry Pi boards.
...
Removed `width` and `height` from the response of the [`GetImage`](/reference/apis/components/camera/#getimage) method in the camera API.
