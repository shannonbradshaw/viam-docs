...
For an introduction to configuring a module on your machine, see [Configure hardware](/hardware/configure-hardware/) instead.
...
| `depends_on`| array | Optional | Deprecated. Use [dependencies](/build-modules/dependencies/) instead. The names of resources that must be available before this resource starts. |
...
- `namespace` is the namespace of your organization, which you can find or [create](/build-modules/module-reference/#create-a-namespace-for-your-organization) on your organization settings page.
  ...
  If you do not intend to [upload your module](/build-modules/deploy-a-module/) to the [registry](https://app.viam.com/registry), you do not need to use your organization's namespace as your model's namespace.
...
| Name | Type | Required? | Description |
| ---- | ---- | --------- | ----------- |
| `type` | string | **Required** | `registry` or `local`, depending on whether the module is in the [registry](https://app.viam.com/registry) or is started [locally](/build-modules/write-a-driver-module/#test-your-module-locally) on the device. |
...
| `executable_path` | string | Local modules only | The path to the module's executable file. Only applicable to, and required for, local modules. Registry modules use the `entrypoint` in the [<file>meta.json</file> file](/build-modules/module-reference/) instead. |
...
Not all of these variables are automatically available on [local modules](/build-modules/write-a-driver-module/#test-your-module-locally); you can manually set variables your module requires if necessary.
...
To configure a module that is uploaded to the Viam Registry but has [visibility](/build-modules/manage-modules/#change-module-visibility) set to **Unlisted** (`public_unlisted`), you need to manually add the module to your configuration:
