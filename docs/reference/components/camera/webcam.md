...
When working with a [camera](/reference/components/camera/) component on the Linux platform, your Linux OS must be able to access the camera properly, and the camera must be configured to use a pixel format that Viam supports.
...
- Ensure that your Linux OS is able to access your camera:
  ...
  1.  Then, [stop `viam-server`](/reference/platform/viam-server/manage-viam-server/#run-viam-server), and verify that your Linux OS is able to access that video device properly:
      ...
      If you receive the error `Device or resource busy` instead, be sure you have [stopped `viam-server`](/reference/platform/viam-server/manage-viam-server/#run-viam-server) first, then re-run the command above.
...
- Ensure that your camera uses a supported pixel format:
  ...
  1.  Then, run the following command:
      ...
      In order to use a camera device with Viam, it must support at least one of the [pixel formats supported by Viam](/reference/components/camera/webcam/#using-format).
...
For more information about the vision service, see [Configure computer vision](/vision/configure/).
{{% /expand%}}
