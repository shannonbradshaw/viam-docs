...
For example, when writing code to manage [simultaneous localization and mapping (SLAM)](/reference/services/slam/) for your machine, it makes sense to use the existing [SLAM API](/reference/apis/services/slam/#api), which provides specific functionality required for generating accurate maps of an environment.
